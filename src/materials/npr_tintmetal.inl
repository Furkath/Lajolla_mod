...
/// NPR variant of DisneyTintMetal. When doublesided=true, the geometric_normal
/// rejection is skipped so backfaces still show specular/metallic color.
/// The shading frame is always flipped to align with dir_in, ensuring
/// the specular math works correctly from either side.

Spectrum eval_op::operator()(const NprTintMetal &bsdf) const {
    if (!bsdf.doublesided) {
    ...
    // Flip the shading frame so frame.n aligns with dir_in.
    // For doublesided backfaces this is essential: it lets the specular
    // lobe work as if the surface were facing the viewer.
...
Real pdf_sample_bsdf_op::operator()(const NprTintMetal &bsdf) const {
    if (!bsdf.doublesided) {
        if (dot(vertex.geometric_normal, dir_in) < 0 ||
                dot(vertex.geometric_normal, dir_out) < 0) {
            return 0;
...
std::optional<BSDFSampleRecord>
        sample_bsdf_op::operator()(const NprTintMetal &bsdf) const {
    if (!bsdf.doublesided) {
        if (dot(vertex.geometric_normal, dir_in) < 0) {
            return {};
        }
...
TextureSpectrum get_texture_op::operator()(const NprTintMetal &bsdf) const {
