...
inline Real GTR2aniso(const Vector3 &hl, Real roughness, Real aniso) {
    Real alpha_min = 0.0001;
    Real aspect = sqrt(1.0 - 0.9 * aniso);
    Real ax = fmax(alpha_min, (roughness * roughness) / aspect);
    Real ay = fmax(alpha_min, (roughness * roughness) * aspect);

    Real term_x = (hl.x * hl.x) / (ax * ax);
    Real term_y = (hl.y * hl.y) / (ay * ay);
    Real denominator = c_PI * ax * ay * pow(term_x + term_y + hl.z * hl.z, 2);
    return 1.0 / fmax(denominator,Real(1e-6));
}
inline Real GGX(Real n_dot_h, Real roughness) {
    return GTR2(n_dot_h, roughness);
}
inline Real D_GTR1(Real n_dot_h, Real alpha) {
    if (alpha >= 1.0) return 1.0 / c_PI; // Singularity
    Real alpha2 = alpha * alpha;
    Real t = 1.0 + (alpha2 - 1.0) * n_dot_h * n_dot_h;
    // (a^2 - 1) / (pi * log(a^2) * (1 + (a^2-1) * cos^2))
    return (alpha2 - 1.0) / (c_PI * log(alpha2) * t);
}
...
inline Real smith_masking_gtr2_aniso(const Vector3 &v_local, Real roughness, Real aniso) {
    if (fabs(v_local.z) <= 1e-6) return 0;
    ...
    Real v2x = v_local.x * v_local.x;
    Real v2y = v_local.y * v_local.y;
    Real v2z = v_local.z * v_local.z;

    // the anisotropic G1 term
    Real Lambda = (sqrt(1.0 + (v2x * ax * ax + v2y * ay * ay) / v2z) - 1.0) / 2.0;
...
inline Vector3 sample_visible_normals_aniso(const Vector3 &local_dir_in, Real roughness, Real aniso, const Vector2 &rnd_param) {
    Real alpha_min = 0.0001;
    Real aspect = sqrt(1.0 - 0.9 * aniso);
    Real ax = fmax(alpha_min, (roughness * roughness) / aspect);
    Real ay = fmax(alpha_min, (roughness * roughness) * aspect);

    // ensure on top of the surface
    if (local_dir_in.z < 0) {
        return -sample_visible_normals_aniso(-local_dir_in, roughness, aniso, rnd_param);
    }

    // transform the incoming direction to the "hemisphere configuration"
    Vector3 hemi_dir_in = normalize(Vector3{ax * local_dir_in.x, ay * local_dir_in.y, local_dir_in.z});

    // sample the projected area of the hemisphere (Heitz 2018)
    Real r = sqrt(rnd_param.x);
    Real phi = 2 * c_PI * rnd_param.y;
    Real t1 = r * cos(phi);
    Real t2 = r * sin(phi);
    
    // vertical scaling for the projection
    Real s = (1.0 + hemi_dir_in.z) / 2.0;
    t2 = (1.0 - s) * sqrt(fmax(0.0, 1.0 - t1 * t1)) + s * t2;
    
    // position on the disk re-project to the hemisphere
    Vector3 disk_N{t1, t2, sqrt(fmax(0.0, 1.0 - t1 * t1 - t2 * t2))};

    // reprojection onto hemisphere in the local frame of hemi_dir_in
    Frame hemi_frame(hemi_dir_in);
    Vector3 hemi_N = to_world(hemi_frame, disk_N);

    // transforming the normal back to the anisotropic ellipsoid
    // re-apply the alpha scaling to stretch the normal back to the correct space
    return normalize(Vector3{ax * hemi_N.x, ay * hemi_N.y, fmax(0.0, hemi_N.z)});
}

inline Vector3 sample_gtr1(Real alpha, const Vector2 &rnd_param) {
    Real alpha2 = alpha * alpha;
    
    // Inverse CDF for GTR1
    Real cos_theta_sq = (1.0 - pow(alpha2, 1.0 - rnd_param.x)) / (1.0 - alpha2);
    Real cos_theta = sqrt(std::clamp(cos_theta_sq, Real(0), Real(1)));
    Real sin_theta = sqrt(std::clamp(1.0 - cos_theta_sq, Real(0), Real(1)));
    
    Real phi = 2.0 * c_PI * rnd_param.y;
    return Vector3(sin_theta * cos(phi), sin_theta * sin(phi), cos_theta);
}
