...
Spectrum vol_path_tracing_4(const Scene &scene, int x,
  ...
  Vector3 nee_p_cache = Vector3{0, 0, 0}; // Position from which we can issue NEE
  Real multi_trans_pdf = 1; // Product PDF of transmittance sampling through index-matching surfaces
  ...
  while (true) {
    ...
    if (scatter) {
      ...
      // Geometry term
      Real G = max(-dot(dir_to_light, point_on_light.normal), Real(0)) / (dist_to_light * dist_to_light);
      
      if (G > 0) {
        // Shadow ray loop: trace through index-matching surfaces, accumulate transmittance
        ...
        while (true) {
          ...
          std::optional<PathVertex> shadow_isect = intersect(scene, shadow_ray, shadow_ray_diff);
          
          Real isect_t = shadow_isect ? distance(shadow_p, shadow_isect->position) : next_t;

          // Attenuate by medium (if any)
          if (shadow_medium_id != -1) {
            const Medium &shadow_medium = scene.media[shadow_medium_id];
            Spectrum shadow_sigma_t = get_sigma_a(shadow_medium, shadow_p) + get_sigma_s(shadow_medium, shadow_p);
          ...
          if (!shadow_isect) {
            ...
            if (shadow_isect->material_id >= 0) {
              ...
              if (is_light(scene.shapes[shadow_isect->shape_id]) &&
                  get_area_light_id(scene.shapes[shadow_isect->shape_id]) == light_id) {
            ...
            shadow_p = shadow_isect->position + shadow_dir * get_intersection_epsilon(scene);
          }
        }
        ...
        if (!blocked && max(T_light) > 0) {
          ...
          Spectrum L_emit = emission(light, -dir_to_light, ray_diff.spread, point_on_light, scene);
          
          // NEE contribution: throughput is already applied, but we need sigma_s
          L_nee = throughput * sigma_s * T_light * G * phase_val * L_emit * w / pdf_nee;
        }
      }
      L += L_nee;
      ...
      std::optional<Vector3> next_dir = sample_phase_function(phase, -ray.dir, u_phase);
      if (!next_dir) break;
      ...
      if (vertex) {
        ...
        if (is_light(scene.shapes[vertex->shape_id])) {
          ...
          if (never_scatter) {
            ...
            Real pdf_nee = light_pmf(scene, light_id) * pdf_point_on_light(hit_light, light_point, nee_p_cache, scene);
            
            // Convert phase sampling PDF to area measure
            Real dist = distance(nee_p_cache, vertex->position);
            Real G = max(-dot(ray.dir, vertex->geometric_normal), Real(0)) / (dist * dist);
    ...
    if (bounces >= scene.options.rr_depth) {
      Real prob = min(max(throughput), Real(0.95));
      if (next_pcg32_real<Real>(rng) > prob) break;
      throughput /= prob;
    }
    bounces++;
    if (max_depth != -1 && bounces >= max_depth) break;
  }
  ...
  while (true) {
    ...
    if (scatter) {
      ...
      if (vertex) {
        ...
        Real shape_w = next_pcg32_real<Real>(rng);
        Vector2 light_uv{next_pcg32_real<Real>(rng), next_pcg32_real<Real>(rng)};
        PointAndNormal point_on_light = sample_point_on_light(light, vertex->position, light_uv, shape_w, scene); // Using vertex->position

        Vector3 dir_to_light = point_on_light.position - vertex->position;
        Real dist_to_light = length(dir_to_light);
        dir_to_light = normalize(dir_to_light);
        
        Real G_light = max(-dot(dir_to_light, point_on_light.normal), Real(0)) / (dist_to_light * dist_to_light);
        
        if (G_light > 0) {
           Vector3 shadow_p = vertex->position + dir_to_light * get_shadow_epsilon(scene); // Surface NEE starts offset
           int shadow_medium_id = medium_id;
           Spectrum T_light = make_const_spectrum(1);
           Real p_trans_dir = 1.0;
           int shadow_bounces = 0;
           ...
           while (true) {
              ...
              if (!shadow_isect) break;
              else {
                  if (shadow_isect->material_id >= 0) {
                      if (is_light(scene.shapes[shadow_isect->shape_id]) &&
                          get_area_light_id(scene.shapes[shadow_isect->shape_id]) == light_id) break;
                      blocked = true; break;
                  }
                  shadow_bounces++;
                  if (max_depth != -1 && bounces + shadow_bounces + 1 >= max_depth) { blocked = true; break; }
                  shadow_medium_id = update_medium(*shadow_isect, shadow_ray, shadow_medium_id);
                  shadow_p = shadow_isect->position + shadow_dir * get_intersection_epsilon(scene);
              }
           }
           ...
           if (!blocked && max(T_light) > 0) {
               Spectrum f = eval(mat, -ray.dir, dir_to_light, *vertex, scene.texture_pool);
               Real pdf_nee = light_pmf(scene, light_id) * pdf_point_on_light(light, point_on_light, vertex->position, scene);
               Real pdf_bsdf_sa = pdf_sample_bsdf(mat, -ray.dir, dir_to_light, *vertex, scene.texture_pool);
               Real pdf_bsdf = pdf_bsdf_sa * G_light * p_trans_dir; // G_light here converts SA to Area
        ...
        std::optional<BSDFSampleRecord> bsdf_sample = sample_bsdf(mat, -ray.dir, *vertex, scene.texture_pool, bsdf_uv, bsdf_w);
        
        if (!bsdf_sample) break;
        
        Vector3 next_dir = bsdf_sample->dir_out;
        Real pdf_bsdf_val = pdf_sample_bsdf(mat, -ray.dir, next_dir, *vertex, scene.texture_pool);
        ...
        if (pdf_bsdf_val > 0) {
            ...
            if (bsdf_sample->eta == 0) {
                 ray = Ray{vertex->position, next_dir, get_intersection_epsilon(scene), infinity<Real>()};
            } else {
                 // Transmission: need to offset origin carefully? 
                 // intersect() handles self-intersection via epsilon, so vertex->position should be fine with epsilon.
                 // update_medium is handled at the END of the ray, so if we refracted INTO object, 
                 // medium updates when we hit the BACK side? No.
                 // If we enter/exit, we need to update medium_id HERE?
                 // In vol_path_tracing, typically the logic is: medium only updates when hitting interface.
                 // But here we ARE at the interface.
                 // If transmission happened, we assume we crossed the boundary.
                 // Usually update_medium is called on intersection. But we are leaving.
                 // Let's assume standard behavior: we just raytrace. If we are inside, standard intersection logic handles it?
                 // Wait, scene.camera.medium_id is tracked.
                 // In path_tracing.h: eta_scale logic.
            }
            // For now, simple ray update
            ray = Ray{vertex->position, next_dir, get_intersection_epsilon(scene), infinity<Real>()};
            
            // Critical Fix: Update medium if we transmit/reflect
            // update_medium uses the ray direction to decide if we are entering/exiting
            medium_id = update_medium(*vertex, ray, medium_id);
